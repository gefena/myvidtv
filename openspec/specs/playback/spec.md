...
When a video finishes, the system SHALL automatically advance to the next item in the current tag view or playlist. The player SHALL load the next item without reloading or re-instantiating the IFrame Player (single-load constraint).
