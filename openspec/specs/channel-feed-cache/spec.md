# channel-feed-cache Specification

## Purpose
Defines the client-side localStorage cache for YouTube RSS feed responses. Stores parsed feed data per channel, refreshes on every successful fetch, serves stale data as fallback when YouTube is unavailable, and LRU-evicts entries when the store exceeds 30 channels.
