...
- [ ] 1.3 Verify `useLayout` returns `"tablet"` on both portrait and landscape orientations for a simulated tablet viewport, and that rotation does not change the returned value.
...
- [ ] 2.2 Confirm that switching between `"phone"`, `"tablet"`, and `"desktop"` values in React DevTools does not cause `PlayerArea` to unmount.
...
- [ ] 3.6 Verify listen mode on tablet: activating listen mode shows the audio-only treatment in the player area and the library remains visible without any fixed overlay or layout shift.
